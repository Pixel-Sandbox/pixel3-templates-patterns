...
import { css, MpText, MpFlex, MpIcon, MpBadge, MpCheckbox, IconName } from '@mekari/pixel3'
...
const getChatBackground = computed(() => {
  if (isChecked.value) {
    return 'var(--mp-colors-background-brand-pressed)'
  }

  if (props.isActive) {
    return 'var(--mp-colors-background-brand-selected)'
  }

  if (isHovered.value) {
    return 'var(--mp-colors-background-neutral-hovered)'
  }
...
const getStatusColor = computed(() => {
  if (props.status === 'unassigned') {
    return 'var(--mp-colors-text-warning)'
  }

  if (props.status === 'assigned') {
    return 'var(--mp-colors-text-information)'
  }

  if (props.status === 'resolved') {
    return 'var(--mp-colors-text-success)'
  }
...
<template>
  <MpFlex
    ...
    px="3"
    cursor="pointer"
    borderBottom="1px solid var(--mp-colors-borders-default)"
  ...
  >
    <MpFlex
      ...
      height="7"
      rounded="full"
      bg="var(--mp-colors-background-neutral-subtle)"
    >
      <div
    ...
    <MpFlex gap="0.5" direction="column" width="full">
      <MpFlex gap="2" justifyContent="space-between" alignItems="center" width="full">
        ...
        <MpText v-else size="label-small" color="var(--mp-colors-text-secondary)" style="flex: none">{{
          timeStamp
        }}</MpText>
      </MpFlex>
      <MpFlex gap="1" justifyContent="space-between" alignItems="center" width="full">
        <MpText v-if="isShowReplies" color="var(--mp-colors-text-secondary)">↩︎</MpText>
        <MpIcon
          v-if="contentType !== 'text'"
          size="sm"
          :name="handleContentIcon()"
        ...
        <MpBadge
          ...
          size="sm"
          style="line-height: 0"
          >{{ contentUnread }}</MpBadge
        >
      </MpFlex>
      <MpFlex gap="2" justifyContent="space-between" alignItems="center" width="full">
        <MpText
          size="label-small"
          color="var(--mp-colors-text-placeholder)"
          is-truncated
          v-tooltip="{
            label: channelName,
            placement: 'right'
